#!/usr/bin/env python3
"""
Summarize INCLUDE-50 nested LOPO batch results.

Expected input:
  experiments/include50_nested_lopo_resume_grid/runs/<subset>/<imputation>/test=...__val=.../result.json

Outputs:
  reports/include50/include50_nested_lopo_raw_runs.csv
  reports/include50/include50_nested_lopo_summary_by_run.csv
  reports/include50/include50_nested_lopo_outer_folds.csv
  reports/include50/include50_nested_lopo_summary_outer.csv
  reports/include50/include50_nested_lopo_pivot_f1.csv
  reports/include50/include50_nested_lopo_latex_table.tex
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Summarize INCLUDE-50 nested LOPO results.")
    parser.add_argument(
        "--results-dir",
        type=Path,
        default=Path("experiments/include50_nested_lopo_resume_grid"),
        help="Directory containing runs/ with result.json files.",
    )
    parser.add_argument(
        "--reports-dir",
        type=Path,
        default=Path("reports/include50"),
        help="Directory where CSV/TEX summaries will be written.",
    )
    parser.add_argument("--expected-runs-per-condition", type=int, default=None, help="Expected runs per subset/imputation. If omitted, progress uses the maximum completed observed per condition.")
    ...
    expected = args.expected_runs_per_condition
    if expected is None:
        # Unknown number of signers in INCLUDE-50; infer a conservative value from completed results.
        # For a precise progress percentage, pass --expected-runs-per-condition n*(n-1).
        expected = int(completed["completed_runs"].max()) if len(completed) else 0
    completed["expected_runs"] = expected
    completed["progress_pct"] = completed["completed_runs"] / completed["expected_runs"] * 100 if expected else 0
    ...
    raw_path = args.reports_dir / "include50_nested_lopo_raw_runs.csv"
    summary_run_path = args.reports_dir / "include50_nested_lopo_summary_by_run.csv"
    outer_path = args.reports_dir / "include50_nested_lopo_outer_folds.csv"
    summary_outer_path = args.reports_dir / "include50_nested_lopo_summary_outer.csv"
    progress_path = args.reports_dir / "include50_nested_lopo_progress.csv"
    pivot_path = args.reports_dir / "include50_nested_lopo_pivot_f1.csv"
    latex_path = args.reports_dir / "include50_nested_lopo_latex_table.tex"
